...
<script type="text/javascript" src="/holosolo/js/jquery-1.11.3.js"></script>
...
<c:import url="../header.jsp"></c:import>
	<div align="center" style="margin-top: 100px;">
		<form action="/holosolo/share.do" method="post" name="write_form" enctype="multipart/form-data">
			...
			<input type="hidden" name="command" value="shareUpdate"> 
			<input type="hidden" name="sno" value="${shareTipDTO.sno}">
			<input type="hidden" name="selectedCategory" value="${requestScope.selectedCategory}">
				...
				<tr>
					<td>
						...
						<c:forEach var="kind" items="${ categoryList }" >
							<c:choose>
								<c:when test="${ shareTipDTO.category == kind }">
						...
						<input type="text" class="form-control-write" id="title" maxlength="20" name="title" maxlength="50" size="67" required="required" value="${shareTipDTO.title }">
					</td>
				</tr>
			</table>
			<textarea name="content" id="content" rows="10" cols="100" style=" height: 412px; width: 100%">${shareTipDTO.content}</textarea>
			<script type="text/javascript">
				var oEditors = [];
